...
#import "DetailTraceViewController.h"

@interface TraceInfoViewController ()<DPRequestDelegate,MJRefreshBaseViewDelegate>
{
...
static NSString * const TraceCellId = @"traceTableCell";
-(void)loadTableView{
    self.TableView=[[UITableView alloc]initWithFrame:CGRectMake(0, 0, fDeviceWidth, fDeviceHeight-NavTopHight-10)];
    self.TableView.delegate=self;
    self.TableView.dataSource=self;
    [self.view addSubview:self.TableView];
    
    self.TableView.backgroundColor=collectionBgdColor;
    [self.TableView registerClass:[TraceInfoTableViewCell class] forCellReuseIdentifier:TraceCellId];
...
- (void)createRequest{
    ...
    [params setValue:@"1" forKey:@"priceInfoType"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TraceInfoTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:TraceCellId forIndexPath:indexPath];
    
    // 将数据视图框架模型(该模型中包含了数据模型)赋值给Cell，
    TraceInfoDeal *dm=_tableDataSource[indexPath.item];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    TraceInfoTableViewCell *svc =(TraceInfoTableViewCell*)[self.TableView cellForRowAtIndexPath:indexPath];
    TraceInfoDeal *dm= [svc praseModelWithCell:svc];
    
    
    DetailTraceViewController *shortCutView=[[DetailTraceViewController alloc]init];
    shortCutView.hidesBottomBarWhenPushed=YES;
    shortCutView.navigationItem.hidesBackButton=YES;
    
    [shortCutView setTraceData:dm];
