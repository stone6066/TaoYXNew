...
@interface ShopListViewController ()<DPRequestDelegate,MJRefreshBaseViewDelegate>
{
...
static NSString * const ShopListCellId = @"ShopListTableCell";
-(void)loadTableView{
    self.TableView=[[UITableView alloc]initWithFrame:CGRectMake(0, 0, fDeviceWidth, fDeviceHeight-TopSeachHigh)];
    self.TableView.delegate=self;
    self.TableView.dataSource=self;
    [self.view addSubview:self.TableView];
    
    self.TableView.backgroundColor=collectionBgdColor;
    [self.TableView registerClass:[ShopListTableViewCell class] forCellReuseIdentifier:ShopListCellId];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ShopListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ShopListCellId forIndexPath:indexPath];
    
    // 将数据视图框架模型(该模型中包含了数据模型)赋值给Cell，
    CmpModel *dm=_tableDataSource[indexPath.item];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ShopListTableViewCell *svc =(ShopListTableViewCell*)[self.TableView cellForRowAtIndexPath:indexPath];
    CmpModel *dm= [svc praseModelWithCell:svc];
   // NSLog(@"%ld---%@---cellUrl:",indexPath.item,dm.deal_url);
    
    DetailShopViewController *shortCutView=[[DetailShopViewController alloc]init];
    shortCutView.hidesBottomBarWhenPushed=YES;
    shortCutView.navigationItem.hidesBackButton=YES;
    
    [shortCutView setCompanyData:dm];
